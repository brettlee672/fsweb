...
(function() {
    var app = {
        data: {},
        user:{},
        networkId:"7107c6ed4fab4d7299b7a03c63be51ec",
        localization: {
            defaultCulture: 'en',
            cultures: [{
                name: "English",
    ...
    app.createPeer = function (api) {
        return "https://" + app.networkId + "-" + localStorage.getItem('peer') + ".us.blockchain.ibm.com:5001" + (api != null ? api : "");
    }

    app.login = function (userName,peer,data,chaincodeid) {
        if (localStorage) {
            localStorage.setItem("peer", peer);
            localStorage.setItem("userName", userName);
            localStorage.setItem("chaincodeid", chaincodeid);
            app.mobileApp.navigate("components/" + peer + "Home/view.html" + (data != null ? data : ""));
        }
    };

    app.rememeberMe = function () {
        if (localStorage) {
            if (localStorage.getItem("rememberKey") != null) {

                var loginType = localStorage.getItem("loginType")

                if (localStorage.getItem("loginType") && localStorage.getItem("userName")) {
                    app.mobileApp.navigate("components/" + loginType + "Home/view.html");
                } else {
